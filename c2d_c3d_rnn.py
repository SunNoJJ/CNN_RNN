...
layer_2 = 20
layer_3 = 8
model_path = "c2d_c3d_rnn/"
model_name = "c2d3d"
train_epoch_step = 9999999999999999999999999999999999999999
train_batch = 500
learning_rate = 0.075
display_step = 20
train_file = "./data/w0w1_train.txt"
...
wight = {
    'h1':tf.Variable(tf.random_normal([108,layer_1]),name = 'w_h1',dtype = tf.float32),
...
def network_structure_CNN(value,cnn_in_batch):
    ...
    CNNtemp1 = tf.reshape(value,[cnn_in_batch[0],4,64,1])
    ## 设置卷卷积核 核高、核宽、图像通道数、卷积核个数
    conv2d_filter_0 = tf.Variable(tf.random_normal([2,5,1,1]),dtype = tf.float32,name = 'conv2d_filter')
    conv2d_filter_1 = tf.Variable(tf.random_normal([3,7,1,2]),dtype = tf.float32,name = 'conv2d_filter')
    
    ## 配置卷积 输入、卷积核、步长、卷积方式
    CNNtemp2_0 = tf.nn.conv2d(CNNtemp1,filter=conv2d_filter_0,strides = [1,1,1,1],padding = 'VALID')#SAME
    ###
    CNNtemp2_1 = tf.reshape(CNNtemp2_0,[cnn_in_batch[0],3,60,1])
    CNNtemp2_2 = tf.nn.conv2d(CNNtemp2_1,filter=conv2d_filter_1,strides = [1,1,1,1],padding = 'VALID')#SAME
    
    CNNtemp3 = tf.reshape(CNNtemp2_2,[cnn_in_batch[0],108])## 元素个数 = 单个卷积结果*卷积核个数
    return tf.nn.relu6(CNNtemp3)
...
def network_structure_RNN(x,w,b,r,old):
    ...
    old['h1'] = net1 = tf.nn.relu6(RNNtmp4)      
