...
wight = {
    ...
    'h3':tf.Variable(tf.random_normal([layer_2,layer_3]),name = 'w_h3',dtype = tf.float32),
    'out':tf.Variable(tf.random_normal([layer_3,output_len]),name = 'w_out',dtype = tf.float32)   
}
...
biase = {
    ...
    'h3':tf.Variable(tf.random_normal([layer_3]),name = 'b_h3',dtype = tf.float32),
...
recursive = {
    ...
    'h3':tf.Variable(tf.random_normal([layer_3,layer_3]),name = 'r_h3',dtype = tf.float32),
...
old_output = {
    ...
    'h3':tf.Variable(tf.random_normal([layer_3]),name = 'o_h3',dtype = tf.float32),
...
def network_structure(x,w,b,r,old):
    ...
    old['h3'] = net3 = tf.nn.relu(tf.add(tf.matmul([old['h3']],r['h3']),tf.add(tf.matmul(old['h2'],w['h3']),b['h3'])))
    old['out']= net4 = tf.add(tf.matmul([old['out']],r['out']),tf.add(tf.matmul(old['h3'],w['out']),b['out']))
    return old['out']
